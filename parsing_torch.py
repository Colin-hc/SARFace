...
def face_split(input, index):
    ...
    # 创建坐标网格
    y_grid = torch.arange(height).view(1, -1, 1).expand(batch_size, -1, width)
    x_grid = torch.arange(width).view(1, 1, -1).expand(batch_size, height, -1)
...
image_path = 'F:/download/512/test_label/1.png'
image = Image.open(image_path)

# Convert image to grayscale (if not already)
image = image.convert('L')

# Resize the image to 128x128 without changing pixel values
image_resized = image.resize((128, 128), Image.NEAREST)
image_array = np.array(image_resized)
image = torch.tensor([np.array(image_resized)])
face_split(image, 0)