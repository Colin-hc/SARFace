...
class SwiftFormerDecoder(nn.Module):
    def __init__(self, layers, embed_dims=None, mlp_ratios=4, upsamples=None, act_layer=nn.GELU, use_layer_scale=True, layer_scale_init_value=1e-5, vit_num=1, **kwargs):
        super().__init__()
        ...
        network = []
        # network.append(
        #     nn.ConvTranspose2d(embed_dims[-1], embed_dims[-1], kernel_size=3, stride=2, padding=1, output_padding=1))
        for i in range(len(layers) - 1, -1, -1):
            stage = Stage(embed_dims[i], i, layers, mlp_ratio=mlp_ratios, act_layer=act_layer, use_layer_scale=use_layer_scale, layer_scale_init_value=layer_scale_init_value, vit_num=vit_num)
            network.append(stage)
            if i <= 0:
                break
            if upsamples[i] or (i > 0 and embed_dims[i] != embed_dims[i - 1]):
                network.append(nn.Sequential(
                    nn.Conv2d(embed_dims[i], embed_dims[i - 1] * 4, kernel_size=3, padding=1),
                    nn.PixelShuffle(upscale_factor=2)
                ))
        network.append(nn.Sequential(
            nn.Conv2d(embed_dims[0], 19 * 16, kernel_size=3, padding=1),
        ...
        for i in range(len(embed_dims)-1):
            self.concat_layers.append(nn.Conv2d(embed_dims[i] * 2, embed_dims[i], kernel_size=1))
    ...
    def forward(self, encoder_features):
        ...
        for i, block in enumerate(self.network):
            if i in {1, 3, 5}:
                x = block(x)
                encoder_feature = encoder_features[len(encoder_features) - 2 - (i // 2)]
                x = torch.cat([x, encoder_feature], dim=1)
                x = self.concat_layers[-i // 2](x)
            else:
                x = block(x)
        return x

class SwiftFormerSR(nn.Module):
...
def create_swiftformer_sr_model(encoder_depth, encoder_width, decoder_depth, decoder_width):
    encoder = SwiftFormer(
        layers=encoder_depth,
        embed_dims=encoder_width,
        downsamples=[True, True, True, True],
        vit_num=1,
        fork_feat=True
    )
    decoder = SwiftFormerDecoder(
        layers=decoder_depth,
        embed_dims=decoder_width,
        upsamples=[True, True, True, True],
        vit_num=1
    )
    return SwiftFormerSR(encoder, decoder)

# model = create_swiftformer_sr_model(SwiftFormer_depth['l1'], SwiftFormer_width['l1'], SwiftFormer_depth['l1'], SwiftFormer_width['l1'])




def validate_model(model, input_shape):
    """
    Validates the SwiftFormerSR model by running a forward pass with dummy data and checking output dimensions.

    Args:
        model: The SwiftFormerSR model to be validated.
        input_shape: The shape of the input tensor (batch_size, channels, height, width).

    Returns:
        bool: True if the output dimensions are correct, False otherwise.
    """
    ...
    if output.shape == expected_output_shape:
        print("Model validation successful: Output dimensions are correct.")
        return True
    else:
        print("Model validation failed: Output dimensions are incorrect.")
        return False

# Example usage
encoder_depth = SwiftFormer_depth['l1']
encoder_width = SwiftFormer_width['l1']
decoder_depth = SwiftFormer_depth['l1']
decoder_width = SwiftFormer_width['l1']
...
# model = SwiftFormerSR(encoder, decoder)
model = create_swiftformer_sr_model(SwiftFormer_depth['l1'], SwiftFormer_width['l1'], SwiftFormer_depth['l1'], SwiftFormer_width['l1'])
# Validate the full model with input shape (batch_size=1, channels=3, height=224, width=224)
# validate_model(model, input_shape=(1, 3, 128, 128))