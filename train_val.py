...
class Trainer(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.seg_model = create_swiftformer_sr_model(SwiftFormer_depth['l1'], SwiftFormer_width['l1'],
                                                     SwiftFormer_depth['l1'], SwiftFormer_width['l1'])
        self.iter = 0
    ...
    def forward(self, images, labels):
        ...
        seg = seg.detach()
        seg = face_split(seg)
        # 分割信息，变成b*23*112*112 的特征，每一个112*112代表对应一个部位的mask
        one_hot = torch.zeros((img.size(0), 23, 128, 128)).to(seg.device)
        one_hot = one_hot.scatter(1, seg.unsqueeze(1), 1)
        seg = torch.nn.functional.interpolate(one_hot, (112, 112), mode='nearest')

        action = torch.randint(0, 2, (seg.size(0), 23), device=seg.device, dtype=torch.float32)
        ...
        embeddings, norms = self.model(prcessed_images)
        cos_thetas, _ = self.head(embeddings, norms, labels)
